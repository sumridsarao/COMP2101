...
#Assigning variablles to the output
hostName=$(hostname)
interface=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
lanAddress=$(ip a s $interface_name|awk '/inet /{gsub(/\/.*/,"");print $2}')
lanhostName=$(getent hosts $(ip a s $interface_name)|awk '/inet /{gsub(/\/.*/,"");print $2}' | awk '{print $2}')
externalIP=$(curl -s icanhazip.com)
externalName=$(getent hosts $external_ip| awk '{print $2}')
routerAddress=$(ip r s default| cut -d ' ' -f 3)
routerhostName=$(getent hosts $router_address|awk '{print $2}')

#displaying the outputs by calling them from their variables.
cat <<EOF
Hostname        : $hostName
