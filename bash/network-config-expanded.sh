...
#####
#this script is displaying system identification summary and interface information
#this code of script will display system identification summary like its hostname, default router, its name, external ip and its name, etc.
...
#####
#this code of script display information about interface like its name, address, network name and address, etc.
...
declare -a array=($interface)
Making a loop for processing all work in array
for i in "${array[@]}"; do
#Putting on the conditions for skipping loopback interface
if [[ $i = lo* ]] ;
		then continue ;
fi
